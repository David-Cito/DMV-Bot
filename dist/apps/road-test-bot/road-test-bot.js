...
async function scanRoadTestAppointments(initialPage, options = {}) {
    const { scanDays = 45, takeScreenshots = true } = options;
...
async function monitorRoadTest(options = {}) {
    const { headless = process.env.ROAD_TEST_HEADLESS !== 'false' && process.env.CI === 'true', slowMo = process.env.CI === 'true' ? 0 : 300, scanDays = 45, } = options;
    ...
    try {
        await context.request.get(exports.START_URL, { timeout: 5000 });
    }
    catch {
...
//# sourceMappingURL=road-test-bot.js.map