...
exports.getSoonestAppointmentForLocation = getSoonestAppointmentForLocation;
exports.finalizeRun = finalizeRun;
exports.appendResultAndFinalizeIfComplete = appendResultAndFinalizeIfComplete;
exports.monitorLocation = monitorLocation;
...
async function getSoonestAppointmentForLocation(page, locationName, opts = {}) {
    ...
    await gear.waitFor({ state: 'hidden', timeout: 60_000 }).catch(() => { });
    const slots = await page.$$eval('.time_wrap .time[data-val]', (els) => els.map((el) => ({
        dataVal: el.getAttribute('data-val') || '',
        text: (el.textContent || '').trim(),
    })));
    if (!slots.length) {
...
async function finalizeRun(results, runAt) {
    ...
    if (resolvedTargetDate) {
        ...
        alerts = results.filter((r) => {
            ...
            if (Number.isNaN(slotTime) || Number.isNaN(targetTime))
                return false;
...
// ============================================================================
async function monitorLocation(locationName, options = {}) {
...
//# sourceMappingURL=monitoring-bot.js.map