...
async function navigateToLocationSelection(page: Page): Promise<void> {
...
async function testLocation(page: Page, locationCode: string): Promise<LocationTestResult> {
  ...
  try {
    ...
    const allLocations = await page.$$eval('.location.button-look.next[data-loc-val]', (els) =>
      els.map(el => ({
    ...
    const allLocationButtons = await page.$$eval('.location.button-look', (els) =>
      els.map(el => ({
        id: el.id,
        class: el.className,
    ...
    const services = await page.$$eval('.transaction.button-look[data-trans-val]', (els) =>
      els.map(el => ({
...
async function main() {
  ...
  if (args.length > 0) {
    ...
    locationsToTest = LOCATION_MAP.filter(
      loc => loc.code.includes(searchTerm) || loc.name.toUpperCase().includes(searchTerm)
    );
  }
