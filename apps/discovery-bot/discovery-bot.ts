...
// Location configuration with data-loc-val codes from the DMV website
// These codes are used to reliably click the correct location button

export interface ServiceConfig {
  name: string;
  transVal: string;  // data-trans-val attribute
}

export interface LocationConfig {
  name: string;
  code: string;  // data-loc-val attribute from the website
  services: ServiceConfig[];
}

// Driver License locations (green buttons on website)
export const DRIVER_LICENSE_LOCATIONS: LocationConfig[] = [
  {
...
export const SATELLITE_DL_LOCATIONS: LocationConfig[] = [
  {
...
export const SATELLITE_OTHER_LOCATIONS: LocationConfig[] = [
  {
...
export const SATELLITE_LOCATIONS: LocationConfig[] = [
...
export const ALREADY_TRACKED_SERVICES: { locationCode: string; servicePattern: string }[] = [
...
export function isServiceAlreadyTracked(locationCode: string, serviceName: string): boolean {
  return ALREADY_TRACKED_SERVICES.some(
    (tracked) =>
      tracked.locationCode === locationCode &&
...
export function getUntrackedLocations(): LocationConfig[] {
...
export interface ServiceSupplyResult {
  locationName: string;
  locationCode: string;
  serviceName: string;
  ok: boolean;
  error?: string;
  // Slot counts by window
  slots30Day: number;      // Slots in days 1-30
  slots60Day: number;      // Slots in days 31-60
  totalSlots: number;      // Total (30 + 60)
  daysWithSlots: number;
  soonestDate: string | null;
  soonestSlotCount: number;
...
export interface LocationSupplyResult {
  locationName: string;
  services: ServiceSupplyResult[];
  scannedAt: string;
}
...
export interface DiscoveryRunResult {
  ...
  failedScans: number;
  locations: LocationSupplyResult[];
}

export interface SupplySummary {
  serviceName: string;
  totalLocations: number;
  locationsWithSupply: number;
  locationsWithNoSupply: number;
  slots30Day: number;       // Total slots in 30-day window across all locations
  slots60Day: number;       // Total slots in 31-60 day window across all locations
  totalSlots: number;
  avgSlotsPerLocation: number;
  soonestDate: string | null;
  // Recommendation based on SCARCITY:
  // 'high' = no availability (high demand) - start monitoring
  // 'medium' = limited availability - worth watching
  // 'low' = some availability - lower priority
  // 'none' = plenty available - users don't need help
...
function ensureDiscoveryDir(): void {
...
function saveDiscoveryResults(result: DiscoveryRunResult): void {
  ensureDiscoveryDir();
...
export async function discoverServiceSupply(
  locationConfig: LocationConfig,
  options: { headless?: boolean; slowMo?: number; timeoutMs?: number } = {}
  ...
  const services: ServiceSupplyResult[] = [];
...
export function analyzeSupply(results: DiscoveryRunResult): SupplySummary[] {
  // Group by service name across all locations
  const serviceMap = new Map<string, ServiceSupplyResult[]>();
  ...
  for (const [serviceName, serviceResults] of serviceMap) {
    ...
    let recommendation: 'high' | 'medium' | 'low' | 'none';
...
export { saveDiscoveryResults, ensureDiscoveryDir };
