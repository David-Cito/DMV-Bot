...
type RunMode = 'discover' | 'monitor' | 'test';
...
function parseArgs(): ParsedArgs {
  ...
  let mode: RunMode = 'monitor';  // Default to monitoring
...
async function runDiscoveryMode(headless: boolean): Promise<void> {
...
async function runMonitoringMode(headless: boolean, days: number, upload: boolean, verbose: boolean): Promise<void> {
  ...
  if (result.summary.earliestDaysAway !== null && result.summary.earliestDaysAway <= INSTANT_ALERT_DAYS) {
    ...
    console.log(`Appointment available in ${result.summary.earliestDaysAway} days (threshold: ${INSTANT_ALERT_DAYS})`);
    console.log('!'.repeat(60));
  }
...
async function runTestMode(headless: boolean): Promise<void> {
  console.log('='.repeat(60));
