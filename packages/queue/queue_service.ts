...
import type { QueueEntry, QueueState, Tier, TimePreference, User, Location } from '../core/types';
import { getSupabaseClient } from '../db/supabase_client';
import { transitionState, type TransitionContext } from './state_machine';
 ...
 */
export async function createQueueEntry(params: CreateQueueEntryParams): Promise<QueueEntry> {
 ...
 */
export async function getQueueEntryByUserAndLocation(
  userId: string,
  locationId: string
...
export interface QueueCounts {
  waiting: number;    // Waitlist
  invited: number;    // Invited, awaiting deposit
  ready: number;      // Pre-queue (deposit paid)
  active: number;     // Active queue
  booking: number;    // Currently being booked
  total_in_queue: number; // ready + active + booking
}

/**
 * Get queue counts for a location
 */
export async function getQueueCounts(locationId: string): Promise<QueueCounts> {
  const supabase = getSupabaseClient();
  ...
  for (const row of data || []) {
    const state = row.state as QueueState;
    if (state in counts) {
      (counts as any)[state]++;
    }
  }
 ...
 */
export async function processQueuePromotions(locationId: string): Promise<{
  promoted_to_active: number;
  invited_to_prequeue: number;
}> {
  const supabase = getSupabaseClient();
  ...
  if (activeSpots > 0 && counts.ready > 0) {
    ...
    const sorted = (readyUsersSorted || []).sort((a, b) => {
      if (a.tier === 'priority' && b.tier === 'flexible') return -1;
      if (a.tier === 'flexible' && b.tier === 'priority') return 1;
      return new Date(a.deposit_paid_at).getTime() - new Date(b.deposit_paid_at).getTime();
    });

    for (const user of sorted.slice(0, toPromote)) {
      const result = await transitionState(user.id, 'active', {
        trigger_type: 'system',
  ...
  if (preQueueSpots > 0 && updatedCounts.waiting > 0) {
    ...
    for (const user of sorted.slice(0, toInvite)) {
      const result = await transitionState(user.id, 'invited', {
        trigger_type: 'system',
 ...
 */
export async function selectUsersForBooking(
  locationId: string,
  slotTime: string,
  botId: string,
  slotId: string,
  ...
  return (data || []).map((row: any) => ({
    ...
    time_preference: row.time_preference as TimePreference,
  }));
}
 ...
 */
export async function releaseUsersFromBooking(
  queueEntryIds: string[],
  reason: string
): Promise<number> {
  if (queueEntryIds.length === 0) return 0;
  ...
  if (error) {
    ...
    for (const id of queueEntryIds) {
      await transitionState(id, 'active', {
 ...
 */
function getTimeBlock(time: string): string | null {
  ...
  if (timeInMinutes >= 8 * 60 && timeInMinutes <= 10 * 60 + 45) return 'morning';

  // Midday: 11:00 - 13:45 (11am - 1:45pm)
  if (timeInMinutes >= 11 * 60 && timeInMinutes <= 13 * 60 + 45) return 'midday';

  // Afternoon: 14:00 - 15:45 (2pm - 3:45pm)
  if (timeInMinutes >= 14 * 60 && timeInMinutes <= 15 * 60 + 45) return 'afternoon';
