...
import { transitionState, bulkTransitionState } from './state_machine';
 ...
 */
export async function resetStuckBookings(timeoutMinutes: number = 10): Promise<number> {
 ...
 */
export async function expireOldInvites(hoursLimit: number = 24): Promise<number> {
  const supabase = getSupabaseClient();
  ...
  const result = await bulkTransitionState(ids, 'waiting', {
 ...
 */
export async function expirePaymentIssues(daysLimit: number = 7): Promise<number> {
  const supabase = getSupabaseClient();
  ...
  const result = await bulkTransitionState(ids, 'expired', {
    trigger_type: 'cleanup',
 ...
 */
export async function processExpiredCancelWindows(): Promise<number> {
  const supabase = getSupabaseClient();
  ...
  const result = await bulkTransitionState(ids, 'confirmed', {
    trigger_type: 'system',
...
export interface CleanupResults {
  stuck_bookings_reset: number;
  invites_expired: number;
  slot_locks_cleaned: number;
  payment_issues_expired: number;
  cancel_windows_processed: number;
  duration_ms: number;
}
 ...
 */
export async function runCleanupJobs(): Promise<CleanupResults> {
